...
style = get_style({"questionmark": "bold red", 
                    "question": "bold red", 
...
try:
    conobj = sql.connect(host="db4free.net", 
                        user="sql6581022", 
                        passwd="shop_password", 
                        database="shop_user"
    )
    cursor = conobj.cursor()
except Exception as e:
    console.print(f"Could not connect to MySQL database \n{e}", style="error")
    spinner = Spinner("point", text=Text("Terminating the Program", style="error"), style="error")
...
def sign_in(user: str = "customer" or "employee") -> bool:
    ...
    while True:
        ...
        if len(rec) == 0:                                                                         
            ...
            ch = inquirer.confirm(message="Do you want to register ?", style=style).execute()
            if ch:
                if user == "customer":
                    customer_register()
            ...
            while c > 0:
                ...
                if bcrypt.checkpw(password, hashed):                                              
                    ...
                    spinner = Spinner("point", text=Text(f"Continuing to {user} screen", style="green"), style="green")
                    with Live(spinner, refresh_per_second=20) as live:
                        for i in range(5):
                            time.sleep(0.2)
                    ...
                    c -= 1
                    console.print("Password Incorrect \nPlease Try again", style="error")
...
def customer_register():
    try:
        os.system('cls')
        console.print(Panel.fit(Text("CUSTOMER REGISTRATION", style="heading", justify="center"), style="border"),justify="center")
        print()
        ...
        console.print(f"Failed to Register \nPlease try again later \n{e}", style="error")
        console.input("[yellow]Enter a key to continue")
...
def employee_register():
    try:
        os.system('cls')
        console.print(Panel.fit(Text("EMPLOYEE REGISTRATION", style="heading", justify="center"), style="border"),justify="center")
        print()
        name = console.input("[input]Enter your name : ")
...
def full_table_create(items):
    full_table = Table(header_style="bold red", box=box.SIMPLE_HEAD, title="Items Table")
    ...
    for i in range(len(items)):
        full_table.add_row(str(items[i][0]), items[i][1], items[i][2], str(items[i][3]), str(items[i][4]))
...
def items_table_create(items):
    ...
    items_table = Table(header_style="bold red", box=box.SIMPLE_HEAD, title="Items Table")
    ...
    for i in range(len(items)):
        items_table.add_row(str(items[i][0]), items[i][1], items[i][2], str(items[i][3]))
...
def bill_table_create(cart):
	...
	for i in range(len(cart)):
		table.add_row(str(i+1), cart[i][1], str(cart[i][4]), str(cart[i][3]), str(cart[i][3]*cart[i][4]))
...
def bill(cart, dt, sub_total, tax, total_cost):
    try:
        ...
        console.print(f"Date and Time of Purchase : [blue]{dt.day}/{dt.month}/{dt.year} - {dt.hour}:{dt.minute}", style="success")
        ...
        ch = inquirer.confirm(message="Do you want to save the bill as a csv file ?", style=style).execute()
        if ch:
            bill_csv(name, email, dt, cart, sub_total, tax, total_cost)

        console.print("Thank you for shopping at our online supermarket!", style="success")	

    except Exception as e:
        console.print(f"Your Bill could not be generated \nPlease try again later \n{e}", style="error")
...
def bill_csv(name, email, dt, cart, sub_total, tax, total_cost):
    try:
        ...
        for i in range(len(cart)):
            wobj.writerow([i+1, cart[i][1], cart[i][4], cart[i][3], cart[i][3]*cart[i][4]])
        ...
        console.print(f"CSV FILE {file_name} has been generated \nPlease check in the program folder \n", style="success")
        file_object.close()

        ch = inquirer.confirm(message="Do you want to open the csv file on your system now ?", style=style).execute()
        if ch:
            try:
                os.startfile(file_name)
                console.print("File opened", style="success")
            except Exception as e:
                console.print(f"Could not open the file \nPlease make sure a default app is set for opening csv files on your system \n{e}", style="error")
    except Exception as e:
        console.print(f"CSV File was not generated \nPlease Try again \n{e}", style="error")
        console.input("[yellow]Enter a key to continue ")
...
def search_buy():
    ...
    try:
        while True:
            os.system('cls')
            console.print(Panel.fit(Text("SEARCH AND BUY", style="heading", justify="center"), style="border"),justify="center")
            search = inquirer.fuzzy(message="Search for products", choices = item_names, style=style, border=True).execute()
            while True:
                flag = False
                qty = IntPrompt.ask("[yellow]Enter quantity")
                if qty == 0:
                    console.print("Quantity cannot be zero \nPlease Enter a non-zero number", style="error")
                else:
                    for item in items:
                        if item[1] == search:
                            if item[4] < qty:
                                console.print(f"Limited Stock left \nPlease enter a value less than {item[4] + 1}", style="error")
                                flag = True
                                break
                    if not flag:
                        break
            ...
            if not ch:
                console.print("All items successfully added to your cart", style="success")
                spinner = Spinner("point", text=Text(f"Continuing to confirm purchase", style="green"), style="green")
                with Live(spinner, refresh_per_second=20) as live:
                    for i in range(7):
                        time.sleep(0.2)
                break
...
def confirm_purchase():
    ...
    os.system('cls')
    console.print(Panel.fit(Text("CONFIRMATION MENU", style="heading", justify="center"), style="border"),justify="center")
    ...
    if ch:
        try:
            ...
            current_order.append(f"{dt.day}/{dt.month}/{dt.year} - {dt.hour}:{dt.minute}")
            ...
            console.print(f"Failed to Confirm your Purchase \nPlease try again later \n{e}",style="error")
            console.input('[success]Enter a key to continue')
...
def edit_quantity(items, cart):
    os.system('cls')
    console.print(Panel.fit(Text("EDIT QUANTITY", style="heading", justify="center"), style="border"),justify="center")
    
    try:
        console.print(cart_table_create(cart))
        item_no = IntPrompt.ask("[yellow]Enter the item no to edit", choices=itemnos_in_cart, show_choices=False)
        for i in range(len(items)):
        ...
        while True:
            ...
            if qty < stock:
                ...
                console.print(f"Please Enter a value less than {stock + 1}", style="error")
                console.input("[yellow]Enter a key to continue")
    except Exception as e:
...
def remove_cart(cart, itemnos_in_cart):
    try:
        console.print(cart_table_create(cart))
        item_no = IntPrompt.ask("[yellow]Enter item no to remove", choices=itemnos_in_cart, show_choices=False)
        ch = inquirer.confirm(message=f"Confirm to remove item no {item_no}", style=style).execute()
        if ch:
            for item in cart:
                if item[0] == item_no:
                    cart.remove(item)
...
def buy():
    try:
        ...
        while True:
            os.system('cls')
            console.print(Panel.fit(Text("SHOPPING MENU", style="heading", justify="center"), style="border"),justify="center")
            ch = inquirer.select(message="Select your choice", style=style, choices=["Confirm Purchase", "Edit your cart", "Cancel Purchase"]).execute()
            ...
            if ch == "Confirm Purchase":
                ...
                while True: 
                    os.system('cls')
                    console.print(Panel.fit(Text("EDIT CART", style="heading", justify="center"), style="border"),justify="center")
                    ch = inquirer.select(message="Select your choice", style=style, choices=["Change Quantity", "Remove an Item", "Return to Shopping Menu"]).execute()
        ...
        console.print(f"Could not access buy menu \nPlease try again later \n{e}", style="error")
        console.input("[yellow]Enter a key to continue ")    


def item_insert():
    os.system('cls')
    console.print(Panel.fit(Text("INSERT ITEM", style="heading", justify="center"), style="border"),justify="center")
...
def remove_item():
    try:
        ...
        item_no = IntPrompt.ask("Enter item no to remove", choices=item_nos, show_choices=False)
        ...
        console.print("Could not remove the item \nPlease try again later \n",e, style="error")
        console.input("[yellow]Enter a key to continue ")
...
def view_items():
    try:
        while True:
            os.system('cls')
            console.print(Panel.fit(Text("VIEW MENU", style="heading", justify="center"), style="border"),justify="center")
            ch = inquirer.select(message="Select your choice", style=style, choices=["Sort by Price", "Group by Category", "View items out of stock", "Return to Customer Menu"]).execute()
            ...
            if ch == "Sort by Price" :
                ...
                ch = inquirer.select(message="Select your category", multiselect=True, style=style, choices=cats).execute()
                selected = str(ch)
                selected = selected[1:-1]
                selected = '(' + selected + ')'

                q = "SELECT item_name, category, price FROM items_table WHERE category IN {}".format(selected)
                cursor.execute(q)
                rec = cursor.fetchall()
                ...
                item_renderables = [Panel(get_content(x), expand=True) for x in range(len(rec))]
        ...
        console.print(f"Could not view items \nPlease try again later \n{e}", style="error")
        console.input("[success]Press any key to continue ")


def edit_items():
    os.system('cls')
    console.print(Panel.fit(Text("EDIT ITEM", style="heading", justify="center"), style="border"),justify="center")
    ...
    for i in items:
        item_nos.append(str(i[0]))
    item_no = IntPrompt.ask("[yellow]Enter item no you want to edit", choices = item_nos, show_choices=False)
    try:
        ...
        console.print(f"Failed to edit item info \nPlease Try again later \n{e}" ,style="error")
        console.input('[success]Enter a key to continue')
...
def search():
    try:       
        ...
        while True:
            os.system('cls')
            console.print(Panel.fit(Text("SEARCH", style="heading", justify="center"), style="border"), justify="center")
            search = inquirer.fuzzy(message="Enter search query : ", style=style, choices=items_names).execute()
            cursor.execute("SELECT * FROM items_table WHERE item_name = '{}'".format(search))
            search_result = cursor.fetchall()
            item = search_result[0]
            console.print(f"\n[success][i]ITEM DETAILS : [/i][/success]\n Item no : {item[0]} \n Item name : {item[1]} \n Category : {item[2]} \n Price : {item[3]} \n", style="blue")
            ch = inquirer.confirm(message="Continue Searching ? ", style=style).execute()
            if not ch:
                break

    except Exception as e:
        console.print("Could not search at the moment \nPlease try again later \n", e, style="error")
        console.input('[success]Enter a key to continue')
...
def edit_customer(rec):
    global email
    os.system('cls')
    console.print(Panel.fit(Text("EDIT CUSTOMER ACCOUNT", style="heading", justify="center"), style="border"),justify="center")
    try:
        f = 0
        cid = rec[0]
        while True:
            ...
            while True:
                ...
                for i in cursor.fetchall():
                    if i[0] == email:
                        console.print("An account with this email already exists \nPlease provide a new email", style="error")
                        spinner = Spinner("point", text=Text(f"Going back to customer account page", style="red"), style="red")
                        with Live(spinner, refresh_per_second=20) as live:
                            for i in range(7):
                                time.sleep(0.2)
                        f = 1
            ...
            spinner = Spinner("point", text=Text(f"Going back to customer account page", style="green"), style="green")
            with Live(spinner, refresh_per_second=20) as live:
                for i in range(7):
                    time.sleep(0.2)
            break
    except Exception as e:
        console.print("Could not update customer details \nPlease try again later \n",e, style="error")
        spinner = Spinner("point", text=Text(f"Going back to customer account page", style="red"), style="red")
        with Live(spinner, refresh_per_second=20) as live:
            for i in range(15):
                time.sleep(0.2)
...
def change_pass(user):
    os.system('cls')
    console.print(Panel.fit(Text("CHANGE PASSWORD", style="heading", justify="center"), style="border"),justify="center")
    f=0
    while True:
        try:
            if f == 1:
            ...
            if bcrypt.checkpw(current_pass, hashed_pass):
                ...
                while True:
                    ...
                    if new_pass == con_pass:
                        ...
                        f=1
                        spinner = Spinner("point", text=Text(f"Continuing customer account page", style="green"), style="green")
                        with Live(spinner, refresh_per_second=20) as live:
                            for i in range(7):
                                time.sleep(0.2)
                        break
                    else:
                        console.print("Confirmation password does not match with your password", style="error")
        except Exception as e:
            console.print(f"Failed to change your password \nPlease Try again later \n{e}", style="error")
            spinner = Spinner("point", text=Text(f"Going back to customer account page", style="red"), style="red")
            with Live(spinner, refresh_per_second=20) as live:
                for i in range(15):
                    time.sleep(0.2)
...
def orders(rec):
    try:
        os.system('cls')
        console.print(Panel.fit(Text("VIEW PAST ORDERS", style="heading", justify="center"), style="border"),justify="center")
        orders_info = eval(rec[3])
        dates = []
        for order in orders_info:
            dates.append(order[0])
        dates = dates[::-1]
        date = inquirer.select(message="Select the date on which you made the purchase", style=style, choices=dates).execute()
        for order in orders_info:
            if order[0] == date:
        ...
        if len(item_nos) == 1:
            q = "SELECT item_no, item_name, category, price FROM items_table WHERE item_no = {}".format(item_nos[0])
        else:
            q = "SELECT item_no, item_name, category, price FROM items_table WHERE item_no IN {}".format(item_nos)
        cursor.execute(q)
...
def delete_account(user, rec):
    os.system('cls')
    console.print(Panel.fit(Text("DELETE ACCOUNT", style="heading", justify="center"), style="border"),justify="center")
    console.print("Deleting account is permanent. This action is [b][i]irreversible[/b][/i]", style="red blink")
    ch = inquirer.confirm(message="Confirm to delete your account ? ", style=style).execute()
    ...
    if ch:
        try:
            ...
            if bcrypt.checkpw(current_pass, hashed_pass):
                conf = Prompt.ask("[yellow]ENTER 'YES' to delete your account or 'NO' to cancel", choices=["YES", "NO"])
                if conf == "YES":
            ...
            console.print(f"Could not delete your account \nPlease Try again later \n{e}")
            console.input("[success]Enter any key to continue")
...
def account_stats(rec):
    try:
        if len(eval(rec[3])) != 0:
            os.system('cls')
            console.print(Panel.fit(Text("ACCOUNT STATS", style="heading", justify="center"), style="border"),justify="center")
            ...
            console.print(f"Last Purchase was made [blue]{(today - last_dt).days} day(s) ago", style="success")

            console.print(f"\nFirst Purchase was made on : [blue]{items[0][0]}", style="success")
            console.print(f"First Purchase was made [blue]{(today - first_dt).days} day(s) ago \n", style="success")

            console.input("[success]Enter any key to continue")
        else:
            console.print("Please make a purchase before visiting this page", style="error")
            spinner = Spinner("point", text=Text(f"Going Back to customer account page", style="red"), style="red")
            with Live(spinner, refresh_per_second=20) as live:
                for i in range(8):
                    time.sleep(0.2)
    except Exception as e:
            console.print(f"Could not access your account stats \nPlease Try again later \n{e}")
            spinner = Spinner("point", text=Text(f"Going Back to customer account page", style="green"), style="green")
            with Live(spinner, refresh_per_second=20) as live:
                for i in range(8):
                    time.sleep(0.2)
...
def customer_account():
    try:
        while True:
            os.system('cls')
            console.print(Panel.fit(Text("CUSTOMER ACCOUNT", style="heading", justify="center"), style="border"),justify="center")
            ch = inquirer.select(message="Select your choice", style=style, choices=["Account Stats", "View Past Orders", "Edit Account Details", "Change Password", "Delete your Account", "Return to Customer Menu"]).execute()
...
def edit_employee(rec):
    global email
    os.system('cls')
    console.print(Panel.fit(Text("EDIT EMPLOYEE ACCOUNT", style="heading", justify="center"), style="border"),justify="center")
    try:
        f = 0
        eid = rec[0]
        while True:
        ...
        console.print("Could not update employee details \nPlease try again later \n",e, style="error")
...
def employee_account():
    while True:
        try:
            os.system('cls')
            console.print(Panel.fit(Text("EMPLOYEE ACCOUNT", style="heading", justify="center"), style="border"),justify="center")
            ch = inquirer.select(message="Select your choice", style=style, choices=["Edit Account Details", "Change Password", "Delete your Account", "Return to Employee Menu"]).execute()
...
while True:
    os.system('cls')
    console.print(Panel.fit(Text("SHOP MANAGEMENT", style="heading", justify="center"), title="Welcome", style="border"),justify="center")
    ch = inquirer.select(message="Select your choice", style=style, choices=["Customer Login", "Employee Login", "Register", "Quit"]).execute()
    ...
    if ch == "Customer Login":
        ...
        if x:
            while True:
                os.system('cls')
                console.print(Panel.fit(Text("CUSTOMER MENU", style="heading", justify="center"), style="border"),justify="center")
                ch = inquirer.select(message="Select your choice", style=style, choices=["Buy Items", "View Items", "Search for an Item", "Account Details","Sign Out"]).execute()
                if ch == "Buy Items":
                    buy()
                elif ch == "View Items":
                    view_items()
            ...
            console.print("Could not authenticate you \nPlease Try again Later", style="error")
            spinner = Spinner("point", text=Text(f"Quitting Program", style="red"), style="red")
            with Live(spinner, refresh_per_second=20) as live:
                for i in range(7):
                    time.sleep(0.2)
            break
        ...
        if x:
            while True:
                os.system('cls')
                console.print(Panel.fit(Text("EMPLOYEE MENU", style="heading", justify="center"), style="border"),justify="center")
                ch = inquirer.select(message="Select your choice", style=style, choices=["Insert a New Item", "Edit an Existing Item", "Search for an Item", "Delete an Item","Account Details","Sign Out"]).execute()
        ...
        while True:
            os.system('cls')
            console.print(Panel.fit(Text("REGISTRATION MENU", style="heading", justify="center"), style="border"),justify="center")
            ch = inquirer.select(message="Select your choice", style=style, choices=["Customer Registration", "Employee Registration", "Return to Main Menu"]).execute()
